...
<template>
  <div class="px-4">
          ...
          <a-form-item class="flex flex-col gap-y-2" v-bind="validateInfos.amount">
            <template #label>
              <AmountLabel
                ...
                :balanceAmount="Format(walletInfo.balanceAmount)"
            ...
            <div class="mt-2 text-gray-400">
              ...
              <span class="float-right"
                >{{ walletInfo.fee }} {{ withdrawState.currency }}</span
              >
            </div>
            <div class="mt-2 text-gray-400">
              <span>{{ t("AhZ8ItHb7nCGWMqoQNgDa") }}:</span>
              <span class="float-right"
                >{{ walletInfo.receiptAmount }} {{ withdrawState.currency }}</span
              >
            </div>
          </a-form-item>
    ...
    <div class="flex justify-center gap-x-2">
      ...
      >
      <a-button type="primary" v-if="step === 2" @click="step = 0">{{
        t("TK_RAyPkWgdIN3oBxXG2o")
      }}</a-button>
      <a-button @click="onView" v-if="step === 2">{{
