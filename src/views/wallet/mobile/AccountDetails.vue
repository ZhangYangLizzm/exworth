...
const showAccountSelect = ref(true)
const options = [
    {
        label: '全部',
        value: undefined,
...
<template>
    <div class="px-4">
        <div class="flex ">
            <div class="flex-grow">
                <a-button type="text" class="text-primary text-base bg-slate-100 " style="border-radius: 24px;">
    ...
    <van-popup v-model:show="showAccountSelect" position="bottom" round>
        <div class="p-4">
            <h3>选择筛选项</h3>
            <div>
                <RadioSelect :options="options" title="類型" />
                <RadioSelect :options="options" title="流水類型" />
            </div>
            <div class="mt-20 mb-10 flex justify-center gap-x-4">
                <a-button size="large">取消</a-button>
                <a-button type="primary" size="large">確定</a-button>
            </div>
        </div>
    </van-popup>
