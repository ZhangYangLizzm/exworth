...
const {
  ...
  getMerchantPrepaidCardList,
  { merchantId, batchOrderKey: "cardKeyIn" },
  { immediate: true }
);
...
<template>
  <ATable
    ...
    :scroll="{ x: 1500 }"
  >
    <template #bodyCell="{ value, record, column }">
      <template v-if="column.key === 'Operation'">
        <ADropdown trigger="hover" class="cursor-pointer">
          <a class="text-primary" @click.prevent>···</a>
          <template #overlay>
            <AMenu>
